...
**Why the web constraint matters:** `@gitrelic/core` uses `execa` to run git commands. A value import from core into the Vite browser build would pull Node.js modules into the browser bundle and break the build.
...
- **Functions**: `runGitrelic`, `analyzeParallelDev`, `analyzeHotspotClustering`
...
1. `pnpm --filter @gitrelic/core build`
2. `pnpm --filter @gitrelic/cli build`
3. `pnpm --filter @gitrelic/web build`
...
6. **Check web safety** — Any new imports in `apps/web/` from `@gitrelic/core`? They MUST be `import type`.
