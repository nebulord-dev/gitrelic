...
describe('analyzeParallelDev', () => {
  ...
  it('excludes files with parallel score below MIN_PARALLEL_SCORE (20)', () => {
    ...
    const commits = Array.from({ length: 10 }, (_, week) =>
      makeCommit({ hash: `a${week}`, authorEmail: 'alice@x.com', date: weekDate(week), files: ['a.ts'] })
    );
    // Add one commit from bob in week 0 to create 1 parallel week
    commits.push(makeCommit({ hash: 'b0', authorEmail: 'bob@x.com', date: weekDate(0), files: ['a.ts'] }));
  ...
  it('ignores files not in the tracked set', () => {
    const commits = [
      makeCommit({ hash: '1', authorEmail: 'alice@x.com', date: weekDate(0), files: ['deleted.ts'] }),
      makeCommit({ hash: '2', authorEmail: 'bob@x.com',   date: weekDate(0), files: ['deleted.ts'] }),
      makeCommit({ hash: '3', authorEmail: 'alice@x.com', date: weekDate(1), files: ['deleted.ts'] }),
      makeCommit({ hash: '4', authorEmail: 'alice@x.com', date: weekDate(2), files: ['deleted.ts'] }),
    ];
