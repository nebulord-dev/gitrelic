...
export function analyzeKnowledgeConcentration(busFactorReport: BusFactorReport): KnowledgeConcentrationReport {
  ...
  const concentrationIndex = totalFiles > 0
    ? Math.round((singleAuthorFiles / totalFiles) * 100)
    : 0;
