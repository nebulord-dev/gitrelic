...
export function ActivityInspector({ report, file }: ActivityInspectorProps) {
  ...
  return (
    <div>
      {/* File header */}
      <div style={{ fontSize: 12, fontFamily: "var(--font-mono)", color: "var(--text-primary)", fontWeight: 600, marginBottom: 4 }}>
        {fileName(file)}
      </div>
      ...
      {velocity && (
        <>
          ...
          <div style={statRow}>
            ...
            <span style={statValue}>{velocity.recentCommits} / {velocity.olderCommits}</span>
          </div>
        </>
      )}
      ...
      {!churn && !velocity && !forensics && (
        <div style={{ color: "var(--text-tertiary)", fontSize: 11, marginTop: 20, textAlign: "center" }}>
