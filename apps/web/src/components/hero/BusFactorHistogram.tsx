...
export type BusFactorTier = 'low' | 'medium' | 'high' | 'critical';

export interface BusFactorBucket {
  rangeStart: number;
  rangeEnd: number;
  count: number;
  tier: BusFactorTier;
...
export interface BusFactorHistogramData {
  buckets: BusFactorBucket[];
  maxCount: number;
  totalFiles: number;
...
// Mirrors getBusFactorRisk in packages/core/src/analyzers/bus-factor.ts.
// Note: this is dominantPercent-only — files with uniqueAuthors === 1 always
...
export function prepareBusFactorHistogramData(
  ...
  return {
    buckets,
    maxCount,
    totalFiles: report.busFactors.files.length,
    highOwnershipCount,
  };
}

const TIER_COLORS: Record<BusFactorTier, string> = {
...
interface BusFactorHistogramProps {
  report: GitrelicReport;
}
...
export function BusFactorHistogram({ report }: BusFactorHistogramProps) {
  ...
  const { buckets, maxCount, totalFiles, highOwnershipCount } = useMemo(
    () => prepareBusFactorHistogramData(report),
    [report],
  );
  ...
  if (totalFiles === 0) {
    return (
      <div ref={containerRef} className="w-full h-full flex flex-col">
        ...
        <HeroCaption
          primary="10-bin histogram · bar height = file count · color = ownership tier (low/medium/high/critical)"
          subtitle="No bus-factor signal in this repo. Either no commits were detected or the analyzer hasn't run yet."
        />
      </div>
    );
  }
    ...
    <div ref={containerRef} className="w-full h-full flex flex-col">
      <div className="flex-1 relative">
        <svg
          ...
          height={svgHeight}
          role="img"
          aria-label={`Dominant-author-percent distribution histogram across ${totalFiles} files. ${highOwnershipCount} ${highOwnershipCount === 1 ? 'file is' : 'files are'} at or above the critical-ownership threshold of ${HIGH_OWNERSHIP_THRESHOLD}%.`}
        >
          <g transform={`translate(${PADDING.left},${PADDING.top})`}>
            {/* Critical-ownership threshold zone shading */}
            <rect
              x={thresholdX}
              y={0}
            ...
            <line
              ...
              strokeDasharray="3 3"
            />
            {/* Threshold sits at the last bucket, so the annotation has no
                room to the right — anchor it to end at the threshold line so
                it reads leftward into the open part of the plot. */}
            <text
              x={thresholdX - 6}
              y={12}
              fontSize={9}
              ...
              textAnchor="end"
            >
              critical (≥{HIGH_OWNERSHIP_THRESHOLD}%) · {highOwnershipCount}{' '}
              {highOwnershipCount === 1 ? 'file' : 'files'}
            </text>
      ...
      <HeroCaption
        primary="10-bin histogram · bar height = file count · color = ownership tier (low/medium/high/critical)"
        subtitle="What's the shape of ownership concentration across the repo? How many files are owned by a single author?"
      />
    </div>
  );
}
